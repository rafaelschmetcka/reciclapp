...
class ReciclappSupabaseUser extends BaseAuthUser {
  ReciclappSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> reciclappSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ReciclappSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
